...
BYTES_REQUESTED_REPORT=`perl -ne 'BEGIN{$n=0;$en=0;}{$n+=$1 if ($en&&/\s*\d+\s+\S+\s+\d+\s+(\d+)\s+\d+/);$en=1 if /\s*pid\s+comm\s+# reads\s+bytes_requested\s+bytes_read/;$en=0 if /^\s*$/} END{print "$n";}' < $LOGS_DIR/script__${script}__report.log`
