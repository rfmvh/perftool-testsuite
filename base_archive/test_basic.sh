...
REGEX_SCRIPT_LINE="\s+\S+\s+$RE_NUMBER\s+\[$RE_NUMBER\]\s+$RE_NUMBER:\s+$RE_NUMBER\s+$RE_EVENT\s+$RE_NUMBER_HEX\s+(?:\w+)|(?:\[unknown\])\s+\($RE_PATH|\[\w+\]\)"
...
$CMD_PERF script -i $CURRENT_TEST_DIR/perf.data | perl -ne 'print "$1\n" if /\(([^\)]+)\)$/' | sort -u | grep -P '^/' > $CURRENT_TEST_DIR/basic_dsos_hit.list
