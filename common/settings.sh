...
# general info utilized
DIR_PATH=`dirname "$(readlink -e "$0")"`

TEST_NAME=`basename $DIR_PATH | sed 's/base/perf/'`; export TEST_NAME
MY_ARCH=`arch`; export MY_ARCH
MY_HOSTNAME=`hostname`; export MY_HOSTNAME
MY_KERNEL_VERSION=`uname -r`; export MY_KERNEL_VERSION
MY_CPUS_ONLINE=`nproc`; export MY_CPUS_ONLINE
MY_CPUS_AVAILABLE=`cat /proc/cpuinfo | grep -P "processor\s" | wc -l`; export MY_CPUS_AVAILABLE

# variables for base_c2c and base_mem
...
$CMD_PERF mem record -e list |& grep -q -P 'ldlat-loads\s+:\s+available' || export LDLAT_LOADS_SUPPORTED="no"
$CMD_PERF mem record -e list |& grep -q -P 'ldlat-stores\s+:\s+available' || export LDLAT_STORES_SUPPORTED="no"

# storing logs and temporary files
